...
/**
 * 简单缓存，无超时实现，默认使用{@link java.util.WeakHashMap}实现缓存自动清理
 *
 * @param <K> 键类型
 * @param <V> 值类型
 * @author Looly
...
public class WeakCache<K, V> implements Iterable<Map.Entry<K, V>>, Cache<K, V>, Serializable {
  ...
  /**
   * 构造，默认使用{@link java.util.WeakHashMap}实现缓存自动清理
   */
  public WeakCache() {
   ...
   * 比如使用{@link java.util.WeakHashMap}则会自动清理key，使用HashMap则不会清理<br>
