...
public abstract class AbstractProxyProvider implements ProxyProvider {
  ...
  public boolean verifyFailed(HttpProxy proxy) {
    ...
    try {
      ...
      log.trace("代理IP无效,Host:{} - Port:{} - errorMsg:{}", proxy.getHost(), proxy.getPort(), e.getMessage());
      return true;
    }
  }
  ...
  public HttpProxy getProxy() {
    ...
    return verify ? randomVerifiedProxy() : randomProxy();
  }
  ...
  public boolean addProxy(HttpProxy proxy) {
    return addProxy(proxy.getHost(), proxy.getPort());
  }
